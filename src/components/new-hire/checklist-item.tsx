...
export function ChecklistItem({ 
  ...
  onEdit?: (item: ChecklistItemData) => void; 
    ...
    return (
      <div className="flex items-start justify-between rounded-md border border-[#D1CECE] bg-white p-4 cursor-pointer min-h-[120px] gap-3" onClick={() => onEdit?.(item)}>
        <div className="flex items-start gap-2 min-w-0 flex-1 overflow-hidden">
          ...
          <div className="leading-tight flex-1 min-w-0 overflow-hidden space-y-1">
            <div className="text-base font-bold text-[#0D141C] line-clamp-2 break-words">{item.title}</div>
        ...
        <Button size="icon" variant="ghost" className="text-[#D64575] shrink-0 mt-0" onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}>

...
export function ExecutiveMemberDetailTable({ executiveId }: ExecutiveMemberDetailTableProps) {
  ...
  if (isLoading) {
    return (
      <Card className="border-[#FFF6F6] p-5 shadow-none">
        <div className="text-center py-8">Loading executive member details...</div>
      </Card>
    );
  }
  ...
  if (isError || !executiveMember) {
    return (
      <Card className="border-[#FFF6F6] p-5 shadow-none">
        <div className="text-center py-8 text-red-500">Failed to load executive member details.</div>
      </Card>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-[#1D1F2C]">{executiveMember.name}</h2>
              <Badge variant="secondary" className="bg-[#FFF1F5] text-[#D64575] border-0 mt-1">
          ...
          <div className="flex gap-2">
            <Button
              onClick={() => router.push(ROUTES.ADMIN.EXECUTIVE_MEMBERS_ID_EDIT(executiveId))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-[#1D1F2C]">Contact Information</h3>
            <div className="space-y-3">
              <div>
                <label className="text-sm text-[#667085]">Email</label>
                <p className="text-sm text-[#1D1F2C]">{executiveMember.email}</p>
              </div>
              {executiveMember.phone && (
                <div>
                  <label className="text-sm text-[#667085]">Phone</label>
                  <p className="text-sm text-[#1D1F2C]">{executiveMember.phone}</p>
                </div>
              )}
              {executiveMember.address && (
                <div>
                  <label className="text-sm text-[#667085]">Address</label>
                  <p className="text-sm text-[#1D1F2C]">{executiveMember.address}</p>
                </div>
              )}
              {executiveMember.city && (
                <div>
                  <label className="text-sm text-[#667085]">City</label>
                  <p className="text-sm text-[#1D1F2C]">{executiveMember.city}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-[#1D1F2C]">Role Information</h3>
            <div className="space-y-3">
              <div>

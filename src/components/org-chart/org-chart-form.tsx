  ...
  React.useEffect(() => {
    ...
    if (isExecutiveRole) {
      ...
      setSelectedBranchDeptIds((prev) => {
        if (prev.length > 1) {
          toast.info(
            "Only managers can be assigned to multiple departments. Keeping only one department."
          );
          return [prev[0]];
        }
        return prev;
          ...
          <div className="border-t border-[#E9EAEB] pt-4">
            <BranchDepartmentSelector
              ...
              onChange={(ids) => {
                ...
                const isManagerRole =
                  selectedRoleObj?.access_level === "manager";
              ...
              allowMultiple={
                availableRoles.find((r) => String(r.id) === selectedRole)
              ...
              if (!isManagerRole) {
                return (
                  <p className="mt-1 text-xs text-muted-foreground ml-[16.666667%] md:ml-0">
                    Only one branch and department can be selected for this
                    role. Select Manager role to manage multiple departments.
                  </p>
                );
              }
              return (
                <p className="mt-1 text-xs text-muted-foreground ml-[16.666667%] md:ml-0">
                  Managers can be assigned to multiple branches and departments.
                  The first department will be used as their employee record
                  location.
                </p>
              );
            })()}

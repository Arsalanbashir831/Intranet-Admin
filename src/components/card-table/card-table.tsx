    ...
    <div className={cn("w-full", className)}>
      ...
      <ScrollArea className={cn("w-full overflow-x-auto")}>
        <div className={cn(isMobile && "min-w-[720px]")}>
          ...
          <div
            className={cn(
              "grid grid-cols-4 items-center gap-3 rounded-md border border-[#EDEEF2] bg-[#F7F7F9] px-4 py-3 text-xs font-medium text-[#667085]",
              headerClassName
            )}>
            ...
            {table.getRowModel().rows.length === 0 && (
              <div className="space-y-3">
                {[0, 1, 2].map((i) => (
                  <div
                    key={i}
                    className="grid grid-cols-4 items-center gap-3 rounded-lg border border-[#E4E4E4] bg-white px-4 py-6"
                  />
                ))}
                <div
                  className={cn(
                    "flex items-center justify-center",
                    noResultsContent ? "-mt-14 mb-5" : "-mt-12 mb-7"
                  )}>
                  <div className="text-center">
                    {noResultsContent ?? (
                      <div className="text-center text-[13px] font-medium text-[#111827]">

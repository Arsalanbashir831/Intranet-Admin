...
import { KnowledgeBaseRow } from "@/types/knowledge";
import { transformFolderToRow } from "@/handlers/knowledge-base-handlers";
  ...
  const columns: ColumnDef<KnowledgeBaseRow>[] = [
      ...
      cell: ({ row }) => {
        ...
        const renderAvatarsWithCount = (
          ...
          const avatarElements = visibleItems.map((item, index) => {
            const name =
              item.emp_name || item.branch_name || item.dept_name || "?";
            const initials = name
              .split(" ")
              .map((n: string) => n[0])
              .join("")
        ...
        if (accessLevel === "Specific Branch Departments") {
          return (
            <div className="flex items-center gap-2">
              <Badge
                variant="secondary"
                className="bg-[#ffdfdf] text-[#D64545] border-0">
                {accessLevelDetails.branchDepartments
                  .slice(0, 3)
                  .map((bd, index) => (
          ...
          return (
            <Badge
              variant="secondary"
              className="bg-[#F0FDF4] text-[#166534] border-0">
              All Employees
            </Badge>
          );
        }
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const folderCreatedByAdmin =
          row.original.originalData.created_by?.is_admin === true;
          ...
          <div className="flex items-center gap-1">
            <span onClick={(e) => e.stopPropagation()}>
              <ConfirmPopover
                ...
                disabled={
                  deletingId === row.original.id || deleteFolder.isPending
                }>
  ...
  return (
    <Card className="border-[#FFF6F6] p-5 shadow-none">
      ...
      <CardTable<KnowledgeBaseRow, unknown>
        ...
        rowClassName={() =>
          "hover:bg-[#FAFAFB] grid-cols-[1.4fr_1fr_1fr_1fr_0.8fr] cursor-pointer"
        }
        onRowClick={(row) =>
          router.push(ROUTES.ADMIN.KNOWLEDGE_BASE_FOLDER_ID(row.original.id))
        }

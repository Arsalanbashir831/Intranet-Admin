...
import { transformEmployeeData } from "@/handlers/department-handlers";
...
export function DepartmentsDetailTable({
  ...
  const employees: DepartmentEmployeeRow[] = React.useMemo(() => {
    return transformEmployeeData(employeesData?.employees?.results);
  }, [employeesData]);
  ...
  const columns = React.useMemo<ColumnDef<DepartmentEmployeeRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <CardTableColumnHeader column={column} title="Date of Joining" />
        ),
        cell: ({ getValue }) => {
  ...
  return (
    <Card
      className={cn("border-[#FFF6F6] p-5 shadow-none overflow-hidden", {
      ...
      <CardTable<DepartmentEmployeeRow, unknown>
        ...
        rowClassName={() =>
          "cursor-pointer hover:bg-[#FAFAFB] grid-cols-[1.4fr_1.6fr_0.8fr_1fr]"
        }
        onRowClick={(row) => {
          router.push(
            ROUTES.ADMIN.ORG_CHART_PROFILE_ID(String(row.original.id))
          );
        }}
        footer={(table) => (
          <CardTablePagination

...
import { UploadItem } from "../components/knowledge-base/upload-queue";
...
export function UploadQueueProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sequential processing simulation; replace with real API
  React.useEffect(() => {
    const current = items.find((q) => q.status === "uploading") || items.find((q) => q.status === "pending");
    if (!current) {
  ...
  const value = React.useMemo(() => ({ items, collapsed, enqueueFiles, clear, remove, setCollapsed }), [items, collapsed, enqueueFiles, clear, remove]);

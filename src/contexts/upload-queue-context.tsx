...
import { UploadStatus } from "@/types/knowledge-base";
...
export function UploadQueueProvider({
  ...
  const uploadFiles = React.useCallback(
    async (files: File[], folderId: number, onSuccess?: () => void) => {
      ...
      try {
        ...
        setTimeout(() => {
          setItems((prev) =>
            prev.filter((item) => !uploadItems.some((ui) => ui.id === item.id))
          );
  ...
  React.useEffect(() => {
    const current =
      items.find((q) => !q.isApiUpload && q.status === "uploading") ||
      items.find((q) => !q.isApiUpload && q.status === "pending");
    if (!current) {
    ...
    if (current.status === "pending") {
      setItems((prev) =>
        prev.map((i) =>
          i.id === current.id ? { ...i, status: "uploading", progress: 0 } : i
        )
      );
    }
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setItems((prev) =>
          prev.map((i) =>
            i.id === current.id ? { ...i, progress: 100, status: "done" } : i
          )
        );
        window.dispatchEvent(
          new CustomEvent("kb:queue-finished-item", {
            detail: {
              id: current.id,
              name: current.name,
        ...
        setItems((prev) =>
          prev.map((i) => (i.id === current.id ? { ...i, progress } : i))
        );
      }
    }, 250);
  ...
  const value = React.useMemo(
    ...
    [items, collapsed, enqueueFiles, uploadFiles, clear, remove]
  );
  return (

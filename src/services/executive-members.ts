...
export async function createExecutive(payload: ExecutiveCreateRequest) {
	...
	const res = await apiCaller<ExecutiveCreateResponse>(
		API_ROUTES.EXECUTIVE_MEMBERS.CREATE,
		"POST",
		formData,
		{},
		"formdata"
...
export async function updateExecutive(
	...
	if (hasFile) {
		...
		const res = await apiCaller<ExecutiveUpdateResponse>(
			API_ROUTES.EXECUTIVE_MEMBERS.UPDATE(id),
			"PUT",
			formData,
			{},
			"formdata"
		...
		if (executiveData.education)
			formData.append("education", executiveData.education);
		if (executiveData.bio) formData.append("bio", executiveData.bio);
		if (executiveData.bio) formData.append("bio", executiveData.bio);
...
export async function uploadExecutiveProfilePicture(
	id: number | string,
	file: File
) {
	const formData = new FormData();
	...
	const res = await apiCaller<ExecutiveUpdateResponse>(
		API_ROUTES.EXECUTIVE_MEMBERS.UPLOAD_PICTURE(id),
		"POST",
		formData,
		{},
		"formdata"
...
export async function deleteExecutiveProfilePicture(id: number | string) {
	await apiCaller<void>(
		API_ROUTES.EXECUTIVE_MEMBERS.DELETE_PICTURE(id),
		"DELETE"
	);
}

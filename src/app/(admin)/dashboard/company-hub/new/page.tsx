...
export default function CompanyHubPage() {
  ...
  const [formData, setFormData] = React.useState<CompanyHubFormData | null>(
    null
  );
    ...
    try {
      ...
      if (formData.permittedBranchDepartments?.length) {
        ...
        payload.permitted_branch_departments =
          formData.permittedBranchDepartments.map(Number);
        ...
        payload.permitted_departments =
          formData.permittedDepartments.map(Number);
      }
      ...
      if (formData.attachedFiles && formData.attachedFiles.length > 0) {
        try {
          ...
          toast.success(
            `${formData.type === "policy" ? "Policy" : "Announcement"} ${
              isDraft ? "saved as draft" : "published"
            } successfully with ${formData.attachedFiles.length} attachment(s)`
          );
        } catch {
          ...
          toast.warning(
            `${formData.type === "policy" ? "Policy" : "Announcement"} ${
              isDraft ? "saved as draft" : "published"
            } successfully, but some attachments failed to upload`
          );
        }
      } else {
        toast.success(
      ...
      toast.error(
        `Failed to ${isDraft ? "save draft" : "publish"} ${
          formData.type === "policy" ? "policy" : "announcement"
        }`
      );
    } finally {
